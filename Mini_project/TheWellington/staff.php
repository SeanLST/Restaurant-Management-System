...
if ($userId > 0) {
    try {
        ...
        // Cashiers, chefs, managers stay in staff panel
        if ($staffRole && strtolower($staffRole['role']) === 'waiter') {
            redirect('waiter.php');
        }
        // If no Staff table entry exists, allow access (for backward compatibility)
    } catch (PDOException $e) {
        // If Staff table doesn't have entry or query fails, continue to staff panel
...
$stmt = $pdo->prepare("
    SELECT 'new_reservation' as type, r.reservation_id as id, 
           COALESCE(u.full_name, 'Guest') as name, 
           r.reservation_date, r.reservation_time, r.reservation_date as created_at
    FROM Reservations r
...
// Note: Kitchen table may not have time_submitted column, using kitchen_id for ordering
$stmt = $pdo->prepare("
    SELECT 'kitchen_order' as type, k.kitchen_id as id, 
           rt.table_number, m.name as item_name, k.quantity,
           NOW() as time_submitted
    FROM Kitchen k
...
// Completed Orders
$stmt = $pdo->prepare("
    SELECT 'order_complete' as type, b.bill_id as id, u.full_name as name,
           b.total_amount, b.payment_method, r.reservation_date
    FROM Bills b
...
$stmt = $pdo->prepare("
    SELECT 
        (SELECT COUNT(*) FROM Reservations WHERE reservation_date = CURDATE()) as today_reservations,
        (SELECT COUNT(*) FROM Kitchen) as today_orders,
        (SELECT COUNT(*) FROM Bills b
         JOIN Reservations r ON b.reservation_id = r.reservation_id
         WHERE DATE(r.reservation_date) = CURDATE()) as completed_bills
");
$stmt->execute();
$stats = $stmt->fetch();
...
$stmt = $pdo->prepare("
    ...
    JOIN Reservations r ON b.reservation_id = r.reservation_id
    JOIN payment_transactions pt ON b.transaction_id = pt.transaction_id
    WHERE YEARWEEK(r.reservation_date, 1) = YEARWEEK(CURDATE(), 1)
    AND pt.payment_status = 'verified'
");
$stmt->execute();
...
$stmt = $pdo->prepare("
    ...
    JOIN Reservations r ON b.reservation_id = r.reservation_id
    JOIN payment_transactions pt ON b.transaction_id = pt.transaction_id
    WHERE YEAR(r.reservation_date) = YEAR(CURDATE()) AND MONTH(r.reservation_date) = MONTH(CURDATE())
    AND pt.payment_status = 'verified'
");
$stmt->execute();
...
$stmt = $pdo->prepare("
    SELECT pt.*, u.full_name, u.phone, rt.table_number
...
$stmt = $pdo->prepare("
    SELECT r.*, rt.table_number, u.full_name, u.phone, m.points as member_points
    FROM Reservations r
    LEFT JOIN restaurant_table rt ON r.table_id = rt.table_id
    LEFT JOIN Users u ON r.user_id = u.user_id
...
$stmt = $pdo->prepare("
    SELECT 
        ...
        MIN(k.order_time) as order_time,
        m.name as item_name, 
        m.category, 
        m.price as item_price,
        rt.table_number,
        MAX(r.reservation_date) as reservation_date,
    ...
    GROUP BY k.reservation_id, k.item_id, k.table_id, k.reservation_time, 
             m.name, m.category, m.price, rt.table_number
    ORDER BY MIN(k.order_time) ASC, COALESCE(k.reservation_time, '00:00:00') ASC
");
$stmt->execute();
...
$stmt = $pdo->query("
    SELECT u.*, m.points, m.membership_id,
