...
<head>
    ...
    <style>
        ...
        /* Restaurant Layout SVG Container */
        .restaurant-layout {
            width: 100%;
            background: #faf8f3;
            border: 3px solid #8b6f47;
        ...
        .table-selected .table-rect {
            ...
            stroke: #f57c00;
            stroke-width: 3;
            filter: drop-shadow(0 4px 8px rgba(255,167,38,0.4));
        }
        
        .table-booked .table-rect {
            fill: #ef5350;
            stroke: #c62828;
            stroke-width: 2;
            opacity: 0.6;
        ...
        .legend-box-full.available {
            background: #66bb6a;
        }
        
        .legend-box-full.selected {
            background: #ffa726;
        }
        
        .legend-box-full.booked {
            background: #ef5350;
        }
...
<nav>
    ...
    <ul class="nav-links">
        ...
        <li><a href="../customer/view_history.php">HISTORY</a></li>
        <?php endif; ?>
    </ul>
            ...
            <div id="step-1" class="form-step">
                ...
                <div class="reservation-fullwidth">
                    <div class="reservation-grid-fullwidth">
                        ...
                        <div class="column-fullwidth">
                            <div class="column-header-full" style="display: flex; justify-content: space-between; align-items: center;">
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    <i class="fas fa-chair"></i>
                                    <h3>Select Table</h3>
                                </div>
                                <button type="button" class="btn-view-layout" onclick="openLayoutModal()">
                                    <i class="fas fa-map-marked-alt"></i> View Restaurant Layout
                                </button>
                            </div>
                            <div class="column-body-full">
                                ...
                                <div class="legend-full">
                                    <div class="legend-item-full">
                                        <span class="legend-box-full available"></span>
                                        <span>Available</span>
                                    </div>
                                    <div class="legend-item-full">
                                        <span class="legend-box-full selected"></span>
                                        <span>Your Selection</span>
                                    </div>
                                    <div class="legend-item-full">
                                        <span class="legend-box-full booked"></span>
                                        <span>Reserved</span>
                                    </div>
                                </div>
                                ...
                                <div class="selected-table-info" id="selectedTableInfo" style="display: none;">
                                    <h4 id="selectedTableTitle">✅ Table Selected</h4>
                                    <p id="selectedTableDetails"></p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                if (selectedTime && data.tables) {
                    ...
                    // Clear selected table info when tables are reloaded
                    const selectedInfoEl = document.getElementById('selectedTableInfo');
                    if (selectedInfoEl) {
...
// Render tables with SVG layout
function renderTables(tables) {
    const container = document.getElementById('table-grid-container');
    
    // Hide selected table info when rendering
    const selectedInfoEl = document.getElementById('selectedTableInfo');
    if (selectedInfoEl) {
        selectedInfoEl.style.display = 'none';
    }
    ...
    // Fetch booked tables from the dedicated endpoint
    fetchBookedTables(selectedDate, selectedTime, (bookedTableNumbers) => {
        // Create a map of table statuses for quick lookup
        const tableStatusMap = {};
        tables.forEach(table => {
            const isBooked = bookedTableNumbers.includes(table.table_number) || table.status === 'occupied';
            tableStatusMap[table.table_number] = {
                booked: isBooked,
        ...
        // Also mark tables as booked if they're in the booked list but not in tables array
        bookedTableNumbers.forEach(tableNum => {
            if (!tableStatusMap[tableNum]) {
                // Get capacity from default table layout (1-2: 2 seats, 3-5,7: 4 seats, 6: 6 seats, 8: 2 seats)
                const capacity = (tableNum === 6) ? 6 : ((tableNum === 1 || tableNum === 2 || tableNum === 8) ? 2 : 4);
                tableStatusMap[tableNum] = {
                    booked: true,
                    capacity: capacity,
                    booked_by: ''
                };
        ...
        renderSVGLayout(tableStatusMap);
    });
}

// Fetch booked tables from the API
function fetchBookedTables(date, time, callback) {
    if (!date) {
        callback([]);
        return;
    }
    
    const url = `get_booked_tables.php?date=${encodeURIComponent(date)}${time ? '&time=' + encodeURIComponent(time) : ''}`; // Already in reservation/ folder, no ../ needed
    
    fetch(url)
        .then(response => response.json())
        .then(data => {
            if (data.success && data.bookedTables) {
                callback(data.bookedTables);
            } else {
                console.warn('Failed to fetch booked tables:', data.error || 'Unknown error');
                callback([]);
            }
        })
        .catch(error => {
...
// Render the SVG layout with table statuses
function renderSVGLayout(tableStatusMap) {
    const container = document.getElementById('table-grid-container');
    
    // Helper function to get table class
    const getTableClass = (tableNum) => {
        const status = tableStatusMap[tableNum];
        if (!status) return 'table-available';
        if (status.booked) return 'table-booked';
        if (selectedTable === tableNum) return 'table-selected';
        return 'table-available';
    };
    
    // Helper function to check if table is clickable
    const isClickable = (tableNum) => {
        const status = tableStatusMap[tableNum];
        return status && !status.booked;
    };
    
    // SVG layout with dynamic table states
    container.innerHTML = `
        <div class="restaurant-layout">
            <svg viewBox="0 0 900 600" xmlns="http://www.w3.org/2000/svg">
                <!-- Main Restaurant Walls -->
                <rect class="wall" x="10" y="10" width="880" height="580" rx="5"/>
                
                <!-- Windows (Top Wall) -->
                <rect class="window" x="150" y="10" width="100" height="15"/>
                <rect class="window" x="350" y="10" width="100" height="15"/>
                <rect class="window" x="550" y="10" width="100" height="15"/>
                
                <!-- Entry Door (Bottom Center) -->
                <g>
                    <rect class="entry-door" x="420" y="575" width="60" height="15" rx="3"/>
                    <text x="450" y="568" class="small-label">ENTRY</text>
                </g>
                
                <!-- Kitchen Area (Top Right) -->
                <g>
                    <rect class="kitchen-area" x="650" y="40" width="220" height="200"/>
                    <text x="760" y="80" class="label-text">KITCHEN</text>
                    <text x="760" y="100" class="icon-text">👨‍🍳</text>
                    <rect x="670" y="120" width="40" height="30" fill="#999" rx="3"/>
                    <text x="690" y="135" style="font-size: 16px;">🔥</text>
                    <rect x="720" y="120" width="40" height="30" fill="#999" rx="3"/>
                    <text x="740" y="135" style="font-size: 16px;">❄️</text>
                    <rect x="770" y="120" width="40" height="30" fill="#999" rx="3"/>
                    <text x="790" y="135" style="font-size: 16px;">🥘</text>
                    <rect x="820" y="120" width="40" height="30" fill="#999" rx="3"/>
                    <text x="840" y="135" style="font-size: 16px;">🍳</text>
                    <rect x="650" y="140" width="15" height="60" fill="#8b6f47" stroke="#5d4e37" stroke-width="2"/>
                    <text x="657" y="170" style="font-size: 10px; fill: white; writing-mode: tb;">PASS</text>
                </g>
                
                <!-- Cashier Counter -->
                <g>
                    <rect class="counter" x="650" y="260" width="220" height="60"/>
                    <text x="760" y="280" class="label-text" fill="white">CASHIER COUNTER</text>
                    <text x="760" y="300" style="font-size: 24px;">💳</text>
                </g>
                
                <!-- Cutlery Station -->
                <g>
                    <rect class="cutlery-station" x="650" y="340" width="220" height="80"/>
                    <text x="760" y="365" class="label-text">CUTLERY STATION</text>
                    <text x="760" y="390" style="font-size: 24px;">🍴</text>
                </g>
                
                <!-- Restrooms -->
                <g>
                    <rect class="restroom" x="650" y="440" width="100" height="130"/>
                    <text x="700" y="465" class="label-text">MEN</text>
                    <text x="700" y="490" style="font-size: 32px;">🚹</text>
                    <rect class="restroom" x="770" y="440" width="100" height="130"/>
                    <text x="820" y="465" class="label-text">WOMEN</text>
                    <text x="820" y="490" style="font-size: 32px;">🚺</text>
                </g>
                
                <!-- Exit Doors -->
                <g>
                    <rect class="exit-door" x="50" y="575" width="50" height="15" rx="3"/>
                    <text x="75" y="568" class="small-label" fill="white">EXIT</text>
                </g>
                <g>
                    <rect class="exit-door" x="800" y="575" width="50" height="15" rx="3"/>
                    <text x="825" y="568" class="small-label" fill="white">EXIT</text>
                </g>
                
                <!-- Service Paths -->
                <path class="service-path" d="M 650 140 Q 500 140 400 200"/>
                <path class="service-path" d="M 650 170 Q 550 170 400 300"/>
                <path class="service-path" d="M 650 200 Q 550 250 400 400"/>
                
                <!-- TABLES - Dynamically rendered based on availability -->
                ${renderTableSVG(1, 2, 80, 80, 100, 70, tableStatusMap)}
                ${renderTableSVG(2, 2, 220, 80, 100, 70, tableStatusMap)}
                ${renderTableSVG(3, 4, 360, 80, 120, 80, tableStatusMap)}
                ${renderTableSVG(4, 4, 510, 80, 120, 80, tableStatusMap)}
                ${renderTableSVG(5, 4, 80, 220, 120, 80, tableStatusMap)}
                ${renderTableSVG(6, 6, 240, 220, 140, 90, tableStatusMap)}
                ${renderTableSVG(7, 4, 420, 220, 120, 80, tableStatusMap)}
                ${renderTableSVG(8, 2, 240, 380, 100, 70, tableStatusMap)}
                
                <!-- Decorative Plants -->
                <g>
                    <circle cx="50" cy="100" r="20" fill="#66bb6a" opacity="0.6"/>
                    <text x="50" y="105" style="font-size: 24px;">🌿</text>
                </g>
                <g>
                    <circle cx="600" cy="500" r="20" fill="#66bb6a" opacity="0.6"/>
                    <text x="600" y="505" style="font-size: 24px;">🪴</text>
                </g>
                <g>
                    <circle cx="50" cy="450" r="20" fill="#66bb6a" opacity="0.6"/>
                    <text x="50" y="455" style="font-size: 24px;">🌱</text>
                </g>
            </svg>
        </div>
    `;
...
// Helper function to render individual table SVG
function renderTableSVG(tableNum, capacity, x, y, width, height, statusMap) {
    const status = statusMap[tableNum];
    const isBooked = status && status.booked;
    const isSelected = selectedTable === tableNum;
    // Table is clickable if it's not booked (or if status is undefined, meaning it's available)
    const isClickable = !status || !status.booked;
    const tableClass = isBooked ? 'table-booked' : (isSelected ? 'table-selected' : 'table-available');
    
    const onClick = isClickable ? `onclick="selectTable(${tableNum}, ${capacity})"` : '';
    
    return `
        <g class="table-group ${tableClass}" data-table="${tableNum}" data-capacity="${capacity}" ${onClick}>
            <rect class="table-rect" x="${x}" y="${y}" width="${width}" height="${height}" rx="8"/>
            <text class="table-number" x="${x + width/2}" y="${y + height/2 - 10}">${tableNum}</text>
            <text class="table-capacity" x="${x + width/2}" y="${y + height/2 + 10}">${capacity} Seats</text>
        </g>
    `;
}
...
function selectTable(tableNumber, capacity) {
    ...
    const tableGroup = document.querySelector(`[data-table="${tableNumber}"]`);
    if (!tableGroup) {
        console.error('Table element not found:', tableNumber);
        return;
    }
    
    if (tableGroup.classList.contains('table-booked')) {
    ...
    document.querySelectorAll('.table-group').forEach(group => {
        ...
        if (!group.classList.contains('table-booked')) {
            group.classList.add('table-available');
        }
    });
    
    // Add selection to clicked table
    tableGroup.classList.remove('table-available');
    tableGroup.classList.add('table-selected');
    ...
    if (summaryTableEl) {
        summaryTableEl.textContent = `Table ${tableNumber}`;
    }
    
    // Update hidden input
    document.getElementById('selected_table').value = `Table ${tableNumber}`;
    
    // Update selected table info display
    const selectedInfoEl = document.getElementById('selectedTableInfo');
    const selectedTitleEl = document.getElementById('selectedTableTitle');
    const selectedDetailsEl = document.getElementById('selectedTableDetails');
    
    if (selectedInfoEl && selectedTitleEl && selectedDetailsEl) {
        selectedInfoEl.style.display = 'block';
        selectedTitleEl.textContent = `✅ Table ${tableNumber} Selected`;
        selectedDetailsEl.innerHTML = `<strong>Capacity:</strong> ${capacity} Seats<br><strong>Table Number:</strong> ${tableNumber}`;
    }
...
function openLayoutModal() {
    ...
    // Update iframe source with current date/time if available
    const iframe = document.getElementById('layoutIframe');
    if (iframe && selectedDate && selectedTime) {
        const currentSrc = '../restaurant_layout.html';
        iframe.src = '../restaurant_layout.html' + `?date=${encodeURIComponent(selectedDate)}&time=${encodeURIComponent(selectedTime)}`;
    } else if (iframe) {
        // Reload iframe to get fresh data
        iframe.src = '../restaurant_layout.html';
    }
}
...
// Add notification animations if not already in styles
...
// Listen for table selection from iframe
window.addEventListener('message', function(event) {
    // Security: verify origin if needed
    // if (event.origin !== window.location.origin) return;
    
    if (event.data && event.data.action === 'tableSelected') {
        selectedTable = event.data.tableNumber;
        selectedTableCapacity = event.data.capacity;
        
        // Update form elements
        const tableDisplay = document.getElementById('table_display');
        if (tableDisplay) {
            tableDisplay.textContent = 'Table ' + selectedTable;
        }
        
        // Also update summary-table if it exists
        ...
        if (selectedTableInput) {
            selectedTableInput.value = 'Table ' + selectedTable;
        }
        
        // Update the SVG layout if visible
        selectTable(selectedTable, selectedTableCapacity);
        
        // Close modal
        closeLayoutModal();
        
        // Show success message
        showNotification('Table ' + selectedTable + ' selected!');
    }
});
...
<!-- Layout Modal -->
<div id="layoutModal" class="modal-overlay" style="display: none;">
