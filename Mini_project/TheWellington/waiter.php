...
if ($hasStatusColumn || $hasConfirmColumn) {
    ...
    $whereClause = "WHERE k.item_id IS NOT NULL AND (r.reservation_date = CURDATE() OR r.reservation_date IS NULL)";
    if ($hasStatusColumn) {
    ...
    $query = "
        ...
        ORDER BY rt.table_number, r.reservation_time
    ";
    ...
    // Fallback: No status or confirm_order columns - show all as preparing
    $stmt = $pdo->prepare("
        SELECT 
            rt.table_number,
            rt.capacity,
...
foreach ($orders as $order) {
    ...
    // Update table status - if ANY order is ready, table is ready
    // If confirm_order = 1 OR status = completed/ready, then it's ready
    if ($order['display_status'] === 'ready' || 
        $order['order_status'] === 'ready' || 
        ($order['is_confirmed'] ?? 0) == 1) {
        // Check if ALL orders for this table are ready
        $allReady = true;
        foreach ($ordersByTable[$tableNum]['orders'] as $tableOrder) {
            if ($tableOrder['display_status'] !== 'ready' && 
...
<body>
                    ...
                    <div class="table-card <?php echo $table['status']; ?>">
                        <div class="table-header">
                            ...
                            <div class="table-status status-<?php echo $table['status']; ?>">
                                <?php echo $table['status'] === 'ready' ? 'Ready to Serve' : 'Preparing'; ?>
                            </div>
                        </div>
                            ...
                            <?php foreach ($table['orders'] as $order): ?>
                                <div class="order-item">
                                    ...
                                    <div style="display: flex; align-items: center; gap: 10px;">
                                        ...
                                        <span class="order-item-status item-<?php echo $order['display_status']; ?>">
                                            <?php echo ($order['display_status'] === 'ready' || $order['order_status'] === 'completed') ? 'Ready' : 'Preparing'; ?>
                                        </span>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        ...
                        <?php else: ?>
                            <button class="serve-btn" disabled>
                                <i class="fas fa-hourglass-half"></i> Preparing...
                            </button>
                        <?php endif; ?>
                    </div>
                <?php endforeach; ?>
    ...
    <script>
        ...
        function markAsServed(tableNumber) {
            if (confirm(`Mark all orders for Table ${tableNumber} as served?`)) {
